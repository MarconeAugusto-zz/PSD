...
plot(wzq/pi*fa/2, 20*log10(abs(hzq)));ylim([-40 10]);
title_txt = ['H(z) - BP - Filtro IIR - Butterworth - N = ' num2str(n)];
title(title_txt);xlabel('Hz');ylabel('dB');
% Máscara
plot([0.01,fs,fs,2000],[0,0,-As,-As]+GdB, '--r')
...
x=10^(-0.1/20);
% valores retirados do Filter Designer
Num = [0.03125 -0.015625 -0.046875 0.0546875 0.0703125 -0.140625 -0.0859375 0.5390625 0.960937500000000 0.5390625 -0.0859375 -0.140625 0.0703125 0.0546875 -0.046875 -0.015625 0.03125];
q=quantizer([8,7]);
xq=quantize(q,x)
binxq=num2bin(q,xq);
N = Num*xq;
