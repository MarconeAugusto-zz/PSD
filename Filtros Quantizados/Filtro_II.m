...
Den = [1 0.877075195312500 1.203308105468750 0.426635742187500 0.249557495117188 0.020889282226563 0.006622314453125]; 
Num = [0.055919647216797 -0.136375427246094 0.237262725830078 -0.275764465332031 0.237262725830078 -0.136375427246094 0.055919647216797];
N = Num;
D = Den;
plot(wz/pi*fa/2, 20*log10(abs(hz)));
grid on;hold on;
[hzq, wzq] = freqz(N, D, linspace(0, pi, 10000));
plot(wzq/pi*fa/2, 20*log10(abs(hzq)));ylim([-60 5]);
title_txt = ['H(z) - BP - Filtro IIR - Chebyshev II - N = ' num2str(n)];
title(title_txt);xlabel('Hz');ylabel('dB');
% Máscara do filtro projetado
plot([0,fs,fs,2000],[-As,-As,0,0], '--r')
...
subplot(212)
plot(wz/pi*fa/2, 20*log10(abs(hz)));ylim([-5 2]);xlim([1250 2000]);
title_txt = ['H(z) - BP - Filtro IIR - Chebyshev II - N = ' num2str(n)];
title(title_txt);xlabel('Hz');ylabel('dB');
grid on;hold on;
...
subplot(211)
% valores retirados do Filter Designer
Num = [0 0 0.0009765625 -0.0009765625 -0.001953125 0.0048828125 0 -0.009765625 ...
    0.0068359375 0.013671875 -0.0205078125 -0.0087890625 0.0400390625 -0.0126953125 ...
    -0.0615234375 0.0693359375 0.0771484375 -0.302734375 0.4130859375 -0.302734375 ...
    0.0771484375 0.0693359375 -0.0615234375 -0.0126953125 0.0400390625 -0.0087890625 ...
    -0.0205078125 0.013671875 0.0068359375 -0.009765625 0 0.0048828125 -0.001953125 ...
    -0.0009765625 0.0009765625 0 0];
N = Num;
plot(w/pi*fa/2,20*log10(abs(h))); grid on;ylim([-90 5]);hold on;
...
subplot(212)
plot(w/pi*fa/2, 20*log10(abs(h)));ylim([-5 2]);xlim([1250 2000]);
title_txt = ['H(z) - BP - FIR - Janela Fixa - N = ' num2str(M*2)];
title(title_txt);xlabel('Hz');ylabel('dB');
grid on;hold on;
plot(wq/pi*fa/2, 20*log10(abs(hq)));
